...
# Tell JAX to compile two separate optimized versions of this function based on use_jumps
@jax.jit(static_argnames=['use_jumps'])
def heun_step_coupled_with_jumps(carry, step_idx, noise_traj, B_field, dt, g, omega_0, kappa, n_spins=1, use_jumps=True):
    S_curr, alpha_curr, weight, key = carry
    
    # Always split the key to maintain deterministic parity, even if jumps are off
    ...
    if use_jumps:
        ...
        S_jumped = S_jumped / jnp.linalg.norm(S_jumped) * jnp.linalg.norm(S_next) 
        ...
        # Bypass all jump math completely
        S_final = S_next
        alpha_final = alpha_next
        weight_final = weight
...
# Add use_jumps to the bundle arguments (default True)
def run_coupled_twa_bundle(keys, t_grid, omega_0, kappa, B_field, g, n_photons_initial, initial_direction, batch_size=1000, n_spins=1, use_jumps=True):
    ...
    def solve_single_trajectory(key):
        ...
        # 4. Setup Carry: Now includes the initial weight of 1.0 and the jump PRNG key
        initial_weight = 1.0
        carry_init = (s0, alpha0, initial_weight, k_jump)
        
        def scan_body(carry, idx):
            return heun_step_coupled_with_jumps(
                carry, idx, noise_traj, B_field, dt, g, omega_0, kappa, n_spins, use_jumps=use_jumps
            )
        ...
        # MUST multiply the trajectory by the accumulated statistical weight!
    ...
    status_msg = "WITH JUMPS" if use_jumps else "CONTINUOUS"
    print(f"Starting Explicit Coupled DTWA ({status_msg}): {n_total} trajectories in {n_batches} batches.")
    
    for i in tqdm(range(n_batches), desc=f"DTWA Batches ({status_msg})"):
