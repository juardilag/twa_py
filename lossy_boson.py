...
from qutip import (tensor, qeye, destroy, sigmax, sigmay, sigmaz, 
                   basis, fock_dm, coherent_dm, mesolve)
...
def solve_dynamics_vacuum(Bx, By, Bz, wa, g, kappa, times, rho0, N=30):
    ...
    # 0.5 factor corresponds to Pauli matrices (S = sigma / 2)
...
def get_initial_state(v, n_photons, N=30):
    """
    ...
    # 2. Boson State (FIXED: Coherent state instead of Mixed Poissonian)
    if n_photons == 0:
        # Exact vacuum state matches the T=0 constraint 
        rho_boson = fock_dm(N, 0) 
    else:
        # True Coherent state |alpha><alpha| with alpha = sqrt(n)
        # This provides the non-zero mean field <a + a_dag> = 2*sqrt(n)
        rho_boson = coherent_dm(N, np.sqrt(n_photons))
